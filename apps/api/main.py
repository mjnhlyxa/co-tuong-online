...
def get_valid_moves(board, row, col, only_captures=False):
    ...
    if ptype == "general":
        ...
        # Move two diagonal, cannot cross river
        ...
        # Move orthogonally, capture requires exactly one screen
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            found_piece = False
            for dist in range(1, max(BOARD_ROWS, BOARD_COLS)):
                new_row, new_col = row + dr * dist, col + dc * dist
...
def count_pieces_between(board, from_row, from_col, to_row, to_col):
    ...
    dr = (to_row - from_row) // max(1, abs(to_row - from_row)) if to_row != from_row else 0
    dc = (to_col - from_col) // max(1, abs(to_col - from_col)) if to_col != from_col else 0

    if dr != 0 and dc != 0:
        return -1  # Not orthogonal
...
def is_move_legal(board, from_row, from_col, to_row, to_col, color):
    """Check if a move is legal (doesn't leave general in check)"""
...
def make_move_on_board(board, from_row, from_col, to_row, to_col):
    """Make a move and return the new board and captured piece"""
    new_board = [row[:] for row in board]
    piece = new_board[from_row][from_col]
...
async def get_game(code: str):
    ...
    if not game:
        # Check if room exists but game hasn't started
        room = rooms_collection.find_one({"code": code.upper()})
        if room:
            return {
                "success": True,
...
async def make_move(code: str, request: MoveRequest):
    ...
    # Check if move is legal (doesn't leave general in check)
    color = piece["color"]
    ...
    # Update time bank (simplified - just subtract a few seconds)
